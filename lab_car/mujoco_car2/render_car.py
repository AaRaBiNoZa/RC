...
def get_wheel1_pos(car_x, car_y, car_z, car_or, rel_x, rel_y, rel_z):
    ninety_by_x = R.from_matrix(np.array([
    [1., 0., 0.],
    [0., 0., 1.],
    [0.,-1.,0.],
    ]))

    cos_ = np.cos(car_or/180 * np.pi)
    sin_ = np.sin(car_or/180 * np.pi)

    ninety_by_z = R.from_matrix(np.array([
        [cos_, -sin_, 0.,],
        [sin_, cos_, 0.,],
        [0., 0., 1.,]
    ]))

    r = ninety_by_z * ninety_by_x

    rvec = r.as_rotvec(degrees=True)
    ...
    pos_x, pos_y, pos_z = (affine_matrix @ np.array([rel_x, rel_y, rel_z, 1.]))[:-1]

    return x_, y_, z_, angle, pos_x, pos_y, pos_z


def create_scene(x, y, car_ori, radar_ori, i):
    rot_x, rot_y, rot_z, angler, pos_x, pos_y, pos_z = get_wheel1_pos(x,y,0.1, car_ori,0.1,0.1,0)
...
<mujoco>
    ...
    <worldbody>
        ...
        <body name="wheel_1" pos="{pos_x} {pos_y} {pos_z}" axisangle="{rot_x} {rot_y} {rot_z} {angler}">
                <geom size="0.07 0.01" rgba="1 0 0 0.9" type="cylinder"/>
        </body>
    </worldbody>
</mujoco>
"""
